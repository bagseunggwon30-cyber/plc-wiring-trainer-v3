...
export const PUBLIC_MISSIONS: readonly MissionDefinitionV2[] = Object.freeze([
  ...
  },
  {
    ...
    description: 'XBF-AH04A의 외부 24V 전원과 전압/전류 채널을 서로 분리해 결선한다.',
    eligibleModes: ['practice', 'prewire'],
    roles: [
      { id: 'dcSupply', label: 'DC 공급원', allowedProfileIds: ['boundary:dc-supply'] },
      { id: 'analogModule', label: 'XBF 아날로그 모듈', allowedProfileIds: ['ls-electric:xbf-ah04a'] },
      { id: 'voltagePeer', label: '전압 신호 경계', allowedProfileIds: ['boundary:load'] },
      { id: 'currentPeer', label: '전류 신호 경계', allowedProfileIds: ['boundary:load'] },
    ],
    initialState: {},
    actions: [
      { id: 'configure-ai0', label: 'AI0을 전압 채널로 확인한다.', kind: 'configure', target: terminal('analogModule', 'I0+') },
      { id: 'configure-ai1', label: 'AI1을 전류 채널로 확인한다.', kind: 'configure', target: terminal('analogModule', 'I1+') },
    ],
    scenarios: [
      { id: 'voltage-channel' },
      { id: 'current-channel' },
    ...
    expectedConnections: [
      {
        ...
        connections: [
          ...
          connection('voltagePeer', '-', 'analogModule', 'I0-', '전압 신호 - → AI0-'),
          connection('currentPeer', '+', 'analogModule', 'I1+', '전류 신호 + → AI1+'),
          connection('currentPeer', '-', 'analogModule', 'I1-', '전류 신호 - → AI1-'),
        ],
      },
    ],
    ...
    hints: hints(
      '전압과 전류 신호는 같은 아날로그 신호라도 입력 방식이 다르다.',
      'DC 공급원, XBF-AH04A, 전압/전류 신호 경계를 찾는다.',
      'I0+는 전압, I1+는 전류 프로토콜로 정의돼 있다.',
