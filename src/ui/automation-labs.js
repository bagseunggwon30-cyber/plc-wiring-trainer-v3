(function () {
  ...
  const LABS = ['palletizer3d', 'servo2', 'mps', 'pneumatic'];
  ...
  const A = {
    ...
    resizeObserver: null, importedLoaded: false
  };
  ...
  function injectUi() {
    ...
    A.hub.innerHTML = `<nav id="al-tabs"><b>🏭 자동화 실습실</b>${[
      ['palletizer3d', '3축 팔레타이징'], ['servo2', '2축 서보'], ['mps', 'MPS 제어'], ['pneumatic', '공압 제어']
    ...
    for (const [key, html] of [['servo2', servoPane()], ['mps', mpsPane()], ['pneumatic', pneumaticPane()]]) {
  ...
  function createRenderer() {
    ...
    A.scenes.servo2 = createServoScene(); A.scenes.mps = createMpsScene(); A.scenes.pneumatic = createPneumaticScene();
  ...
  function importState(saved) {
    ...
    A.state = { schemaVersion: 1, activeLab: LABS.includes(saved.activeLab) ? saved.activeLab : 'servo2', labs: { servo2: Servo.createState({ saved: saved.labs?.servo2 }), mps: MPS.createState({ saved: saved.labs?.mps }), pneumatic: Pneumatic.createState({ saved: saved.labs?.pneumatic }) }, editor: saved.editor || null };
    ...
    A.activeLab = A.state.activeLab; setLab(A.activeLab); updateScenes(); updateUi(true); persist(true);
  }
  ...
  window.PLCTrainerAutomationLabs = { version: '2.7.1', setVisible, setLab, renderActive, resize, exportState, importState, getEditor, getSceneDiagnostics, get activeLab() { return A.activeLab; }, get state() { return A.state; } };
  init();
})();
