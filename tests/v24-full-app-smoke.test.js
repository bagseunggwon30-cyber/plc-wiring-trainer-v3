...
test('full application script initializes with all device packs and modular runtimes in a DOM smoke harness', () => {
  ...
  const result=vm.runInContext(`({devices:Object.keys(LIB).length,goals:GOALS.length,hasXBL:!!LIB['XBL-C41A'],hasPD:!!LIB['XBF-PD02A'],hasMps:!!LIB['MPS-CONVEYOR-STATION'],pack:window.V27_DEVICE_PACK_REPORT,runtimes:!!(window.PLCTrainerRack&&window.PLCTrainerAnalog&&window.PLCTrainerModbus&&window.PLCTrainerDrive),configUi:!!window.PLCTrainerDeviceConfig})`,context);
  assert.ok(result.devices>=50);
  assert.ok(result.goals>=7);
  assert.equal(result.hasXBL,true);
  assert.equal(result.hasPD,true);
  assert.equal(result.hasMps,true);
  assert.equal(result.runtimes,true);
  assert.equal(result.configUi,true);
  assert.deepEqual([...result.pack.packs],['ls-xgb-v24','ls-xgb-expansion-v27','automation-equipment-v27']);
