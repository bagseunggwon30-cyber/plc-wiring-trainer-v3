...
const bindings = [
  ioBinding('start-input', 'P03', 'B0S00.IN03', 'input', false),
  ioBinding('stop-input', 'P02', 'B0S00.IN02', 'input', true),
  ioBinding('run-output', 'P20', 'B0S00.OUT00', 'output', false),
];
...
function workshop(options: { coilReturn?: boolean; outputCommon?: boolean; plcPower?: boolean } = {}): WorkshopDocumentV3 {
  ...
  const branches: ElectricalBranch[] = [
    ...
    branch('output-to-coil', 'plc-p20-contact', 'P20', 'relay-coil', '14'),
  ...
  if (outputCommon) branches.push(branch('output-com', 'dc24', '+24V', 'plc-p20-contact', 'COM0'));
  if (!coilReturn) {
  ...
  return {
    ...
    elements: [
      ...
      { kind: 'contact', id: 'start-pb', terminalA: '13', terminalB: '14', stateKey: 'operator:start', normally: 'open' },
      { kind: 'contact', id: 'stop-pb', terminalA: '21', terminalB: '22', stateKey: 'operator:stop-closed', normally: 'closed' },
      ...
      { kind: 'contact', id: 'plc-p20-contact', terminalA: 'COM0', terminalB: 'P20', stateKey: 'plc:P20', normally: 'open' },
    ...
    branches,
    reviewScope: { elementIds: ['plc-power', 'start-pb', 'stop-pb', 'plc-start-input', 'plc-stop-input', 'plc-p20-contact', 'relay-coil', 'relay-no1', 'run-lamp'] },
  };
}
...
const definition: XbcRelayLampSliceDefinition = {
  ...
  plcOutputContactStateKey: 'plc:P20',
...
async function connectedAdapter(): Promise<MockPlcRuntimeAdapter> {
  const adapter = new MockPlcRuntimeAdapter(({ inputs, previousOutputs }) => ({
    'run-output': inputs['stop-input'] === true
      && (inputs['start-input'] === true || previousOutputs['run-output'] === true),
  }));
...
describe('XBC relay/lamp runtime vertical slice', () => {
  it('runs start NO -> PLC latch -> P20 dry contact -> MY2N coil -> lamp, then releases through stop NC', async () => {
    ...
    expect(started.frame.plcInputs).toMatchObject({ 'start-input': true, 'stop-input': true });
    ...
    expect(stopped.frame.plcInputs['stop-input']).toBe(false);
  ...
  it('keeps the physical load off when P20 is ON but the coil return is open', async () => {
    const adapter = await connectedAdapter();
  ...
  it.each([
    ['output COM open', { outputCommon: false }, 'OPEN_SOURCE_PATH'],
    ['PLC power open', { plcPower: false }, 'PLC_POWER_UNAVAILABLE'],
  ])('fails safe with %s', async (_name, options, expectedCode) => {
    const adapter = await connectedAdapter();
    const result = await runXbcRelayLampFrame(adapter, {
      frameNumber: 1,
