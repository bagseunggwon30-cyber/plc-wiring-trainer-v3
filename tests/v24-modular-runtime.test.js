...
test('v3 HTML loads the modular equipment and runtime adapters without replacing the v3 engine', () => {
  const html = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
  for (const script of ['device-pack-registry.js', 'ls-xgb-v24-pack.js', 'ls-xgb-expansion-pack.js', 'rack-runtime.js', 'analog-runtime.js', 'modbus-runtime.js', 'drive-runtime.js', 'device-config.js']) {
    assert.match(html, new RegExp(script.replace('.', '\\.')));
  }
  assert.match(html, /V27_DEVICE_PACK_REPORT/);
  assert.match(html, /replaceExisting:false/);
  assert.match(html, /전기 판정은 계속 v3 엔진만 사용한다/);
...
test('XBL-C41A pack uses the official TX/RX terminal names and two-wire bridge metadata', () => {
  const { library } = loadPackLibrary();
  const definition = library['XBL-C41A'];
  assert.deepEqual(terminalIds(definition), ['TX+', 'TX-', 'RX+', 'RX-', 'SG']);
  assert.equal(definition.rs485BusPorts[0].requireBridgeIn2Wire, true);
  assert.equal(terminalIds(definition).some(id => ['SDA', 'RDA', 'SDB', 'RDB'].includes(id)), false);
});
...
test('v3 package ships modular sources through the renderer build and exposes device settings', () => {
  const pkg = JSON.parse(fs.readFileSync(path.join(ROOT, 'package.json'), 'utf8'));
  assert.equal(pkg.version, '3.0.4');
  assert.ok(pkg.build.files.includes('build/renderer/**/*'));
  const vite = fs.readFileSync(path.join(ROOT, 'vite.config.ts'), 'utf8');
  assert.match(vite, /'src\/device-packs'/);
  assert.match(vite, /'src\/runtime'/);
  const html = fs.readFileSync(path.join(ROOT, 'index.html'), 'utf8');
  assert.match(html, /PLCTrainerDeviceConfig/);
  assert.match(html, /devId,device:dev,definition:def/);
  assert.match(html, /이 장비에는 변경 가능한 설정이 없습니다/);
