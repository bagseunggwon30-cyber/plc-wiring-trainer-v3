...
- 고급 도구의 **XG-SIM 공식 인터페이스 진단**은 프로젝트 해시와 주소를 사용자가 명시해야 활성화됩니다. `15번` 프로젝트에서 현재 증명된 경로는 `B0S00.IN03/P00003 → M00100 → P00021/B0S00.OUT01`입니다. 계획상의 `P20/OUT00`으로 자동 치환하지 않습니다.
...
- 도메인 첫 수직 fixture는 시작 NO, 정지 NC, XBC P20 무전압 출력, MY2N-D2 14(+)/13(0V) 코일과 표시등을 연결합니다. Mock 래더와 실제 회로 해석을 분리해 P20이 ON이어도 PLC 전원·COM·공급·귀로가 틀리면 코일과 표시등은 OFF입니다.
...
powershell -NoProfile -ExecutionPolicy Bypass -File tools/Get-XgSimProjectIdentity.ps1 `
  -ProjectPath 'C:\XG5000\Projects\15번\15번.xgwx'
```
