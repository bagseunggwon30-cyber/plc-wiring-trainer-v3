...
- 원본에 장비·전선·branch가 있는데 변환 결과 개수가 줄거나 0개가 되면 성공으로 처리하지 않고 격리합니다.
- 해시가 있는 완전한 V3 문서는 canonical 해시가 일치해야 합니다. 손상되거나 지원하지 않는 문서는 원본을 삭제하지 않고 `%LOCALAPPDATA%\PLC Wiring Trainer\Quarantine`에 복사합니다.
